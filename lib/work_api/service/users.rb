...
module WorkApi
  class Service
    module Users
      ...
      #
      # @param [WorkApi::User] user: the user must contain the <tt>arbitrary_id</tt> and <tt>environment_guid</tt>
      # @param [String] path_to_private_key: The local path to the environment's private key file
      #
      # @raise [RemoteError]
      #
      # @return [WorkApi::User] the user parsed from the API's response. **It is possible this won't match the input <tt>user</tt>**
