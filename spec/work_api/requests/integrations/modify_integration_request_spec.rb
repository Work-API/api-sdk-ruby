...
RSpec.describe(WorkApi::Requests::Integrations::ModifyIntegrationRequest) do
  include_context 'with live client'

  let(:integration_id) { 'doesnt_exist' }
  let(:integration_update) { WorkApi::Integration.new(provider: provider, media_type: media_type) }
